...
def main(model_name, weights):
    ...
    if weights is not None:
        ...
        model = timm.create_model(models[model_name], num_classes=0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-w', '--weight', required=False, help='Path to the finetuned weights in the same order as the models.')
    args = parser.parse_args()

    model = args.model
    weight = args.weight

    train_features_file = '_'.join([model, dataset, 'train']) + '.pkl'
    test_features_file = '_'.join([model, dataset, 'test']) + '.pkl'
    features_folder = "./cifar10_features/"
