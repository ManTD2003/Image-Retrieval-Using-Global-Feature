...
from utils.metric_train_utils import train, test
...
def main(args):
    ...
    for model_name in args.models:
        ...
        model_save_path = os.path.join(MODEL_SAVE_FOLDER, f"{model_name}_cifar100.pth")
        ...
        dataset1 = datasets.CIFAR100(root=DATASET_ROOT, train=True,  download=True, transform=transform)
        dataset2 = datasets.CIFAR100(root=DATASET_ROOT, train=False, download=True, transform=transform)
        ...
        test_loader  = torch.utils.data.DataLoader(
            dataset2, batch_size=args.batch_size, shuffle=False,
            num_workers=num_workers, pin_memory=True, persistent_workers=True,
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Metric learning fine-tuning on CIFAR-100")
